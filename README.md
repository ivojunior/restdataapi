...
- `status` (opcional, valores `aberta`/`encerrada`): filtra pelo status da carga (`DAK_ACECAR`) — `encerrada` (códigos `7` ou `8`) ou `aberta` (códigos `1` a `6`, enumeração explícita, não "qualquer outro código"). Sem o parâmetro, traz cargas de qualquer status. `DAK_ACECAR` é uma customização desta instalação do Protheus (no dicionário padrão é "Acerto de Carga Ok?", campo `C(1)` sem lista pública de valores) — os códigos foram confirmados pelo usuário, não por documentação oficial. O filtro é resolvido no banco (`IN`), não no cliente, para não trafegar o volume completo do relatório a cada consulta. O nome do parâmetro (`encerrada`) é mantido por retrocompatibilidade — o texto de fato devolvido no campo `status_carga` (veja abaixo) é `"Fechada"`, igual ao `CASE` de `select_cargas.sql`. Um `DAK_ACECAR` fora das duas listas (ou nulo) não casa com nenhum dos dois filtros — a carga continua aparecendo na listagem sem filtro, mas com `status_carga` nulo.

Assim como `select_cargas.sql`, os campos `caminhao` e `status_carga` já vêm formatados pela própria consulta (via `CASE`/`case()` do SQLAlchemy, calculado no banco — sem custo adicional no plano de execução, pois não entra em `JOIN`/`WHERE`): `caminhao` substitui a placa `KHA0902` por `"Cliente"` (retirada pelo próprio cliente, sem caminhão terceirizado) e nunca é nulo; `status_carga` já vem como `"Aberta"`/`"Fechada"`, mas — diferente de `caminhao` — o `CASE` **não tem `ELSE`**, então pode ser nulo (código fora de `1`-`8`). Isso evita que cada client desktop precise reclassificar essas colunas linha a linha depois de carregar os dados.
