...
| GET    | `/cargas/`                         | Relatório de cargas (baseado em `select_cargas.sql`): itens de carga (DAI070) com veículo (DAK070), nome do cliente (SA1070), valor da nota fiscal (SE1070) e, opcionalmente, percurso (DA5070) e motorista (DA4070); filtra por `data_inicial`/`data_final`/`status` |
...
**Paginação em duas fases.** Diferente dos outros relatórios (uma única query), `/cargas/` resolve a página em duas consultas (`_filtro_cargas` + `_projecao_cargas` em `app/routers/cargas.py`): a primeira só encontra os `R_E_C_N_O_` (chave primária de `DAI070`) da página, usando somente os joins obrigatórios (`DAK070`, `SA1070`) — sem a subquery correlacionada de `SE1070` nem os `LEFT JOIN` de `DA5070`/`DA4070`, que não influenciam quantidade nem ordem das linhas; a segunda busca a projeção completa (todas as colunas, joins de apoio, subquery de valor) filtrando só pelos `R_E_C_N_O_` já escolhidos (busca por chave primária). Isso existe porque, numa única query, o SQL Server pode escolher um plano que calcula a subquery/joins de apoio para **todas** as linhas que casam no período antes de aplicar `OFFSET`/`FETCH` — não só as da página — se não houver um índice que sustente `ORDER BY DAI_FILIAL, DAI_COD, DAI_SEQCAR` junto do filtro de data. Separar em duas fases garante que o trabalho caro rode só para as linhas da página, independente do plano escolhido pelo otimizador.

Isso não substitui índices adequados nas tabelas do Protheus (que esta API não gerencia — são de responsabilidade de quem administra o SQL Server). Índices recomendados, mapeados às colunas usadas em `JOIN`/`WHERE`/`ORDER BY` desta consulta:
...
`SA1070`, `DA5070` e `DA4070` são tabelas de cadastro (cliente/percurso/motorista) geralmente já indexadas por `FILIAL`+`COD` como chave; confirme com `SET STATISTICS IO/TIME` ou o "Actual Execution Plan" do SSMS antes/depois de aplicar, seguindo o mesmo método já usado para decidir a estratégia da subquery de `SE1070` (ver comentário em `_projecao_cargas`).
