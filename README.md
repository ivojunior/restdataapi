...
Baseado no `SELECT` de `select_faturamento.sql`: uma agregação em **duas camadas**. A subconsulta interna calcula, por linha de `SD2070` (item de nota fiscal de saída), a quantidade vendida (`QTDE`), o faturamento (`FATURAMENTO`, zerado para bonificação — veja abaixo) e o custo (`CUSTO_MEDIO`, `SUM(D2_CUSTO1)`), sempre juntando com `SB1000` (produto, join obrigatório por filial/código — `B1_FILIAL = '  '`, `B1_TIPO = 'PA'`: só produtos acabados; item sem produto correspondente do tipo `PA` não aparece). A consulta externa soma tudo por **filial, dia do mês e produto**, e só então calcula `preco_medio` e `lucro_bruto` a partir do `faturamento` já agregado (veja abaixo). Réplica exata dessa estrutura de duas camadas no SQLAlchemy (`app/routers/faturamento.py`), em vez de agregar tudo de uma vez. Aplica como regra de negócio fixa (sempre ativa):
...
Para itens com `D2_YOPER = '542'/'543'/'544'` (bonificação), o `faturamento` entra como **zero** — dar um produto de bonificação não gera receita. `quantidade`, porém, conta normalmente (sempre `D2_QUANT/B1_CONV`, sem exceção) — uma bonificação aumenta a quantidade movimentada do produto sem aumentar o faturamento do grupo. `preco_medio` e `lucro_bruto` são calculados a partir do `faturamento` já zerado (`preco_medio = AVG(faturamento/qtde)`, `lucro_bruto = SUM(faturamento) - SUM(custo)`) — **não** a partir do `D2_TOTAL` bruto: correção feita pelo usuário em `select_faturamento.sql` (a versão anterior calculava os dois a partir do `D2_TOTAL` bruto, ignorando a zeragem de bonificação, o que dava lucro positivo mesmo com faturamento reportado como zero). Com a correção, um item de bonificação sempre contribui com `preco_medio = 0` e `lucro_bruto = -custo` (prejuízo igual ao custo do produto doado).

**`dia` é o dia do mês da emissão (`DAY(D2_EMISSAO)`, 1 a 31) — não uma data completa.** Se o período (`data_inicial`/`data_final`) atravessar mais de um mês, dias iguais de meses diferentes são somados no mesmo grupo (ex.: vendas do dia 5 de janeiro e do dia 5 de fevereiro aparecem juntas como um único "dia 5"). Comportamento herdado de `select_faturamento.sql`, não uma escolha desta API. `DAY(D2_EMISSAO)` é replicado com `CAST(SUBSTRING(D2_EMISSAO, 7, 2) AS INTEGER)` (`D2_EMISSAO` é string `AAAAMMDD`, não uma coluna `DATE`) — o `7` e o `2` **precisam** compilar como literais SQL, não como parâmetros ligados (`?`): a mesma expressão aparece tanto na lista de colunas quanto no `GROUP BY` da subconsulta interna, e o SQL Server só reconhece as duas ocorrências como a mesma expressão (exigido para validar o `GROUP BY`) se o texto for byte-a-byte idêntico — com parâmetros, cada ocorrência recebe um `?` de posição diferente, e o SQL Server rejeita a query com "column is invalid in the select list because it is not contained in either an aggregate function or the GROUP BY clause", mesmo a expressão sendo logicamente igual. Não reproduzível no SQLite dos testes (mais permissivo nessa validação) — descoberto rodando contra o SQL Server real. Ver `literal_column` em `_query_faturamento` (`app/routers/faturamento.py`).
...
> **Risco conhecido, herdado da consulta original:** `margem` (`(lucro_bruto / SUM(faturamento)) * 100`) e, agora também, `preco_medio` (`AVG(faturamento/qtde)`) dividem sem proteção contra zero. Como `faturamento` é sempre zero para bonificação (veja acima), qualquer grupo (filial/dia/produto) com **só bonificação e nenhuma venda** no período tem faturamento total zero, e `qtde` de uma linha individual também pode ser zero se `D2_QUANT` vier zerado. Quando isso acontece, o SQL Server lança um erro de divisão por zero e a requisição falha com `500`. Não foi corrigido aqui (não testável contra SQLite, que não reproduz esse erro — divide por zero retorna `NULL` em vez de lançar exceção) para não alterar o comportamento de `select_faturamento.sql` sem confirmação; se isso acontecer na prática, a correção é envolver os denominadores em `NULLIF(..., 0)`.
